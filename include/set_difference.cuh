...
template <typename T = vidType>
__forceinline__ __device__ T difference_num(T* a, T* source, T size_a, T* b, T size_b) {
...
template <typename T = vidType>
__forceinline__ __device__ T difference_num(T* a, T* source, T size_a, T* b, T size_b, T upper_bound) {
  return difference_num_bs_cache(a, source, size_a, b, size_b, upper_bound);
}
...
template <typename T = vidType>
__forceinline__ __device__ T difference_set_bs_cache(T* a, T* source, T size_a, T* b, T size_b, T* c) {
  //if (size_a == 0) return 0;
  int thread_lane = threadIdx.x & (WARP_SIZE-1); // thread index within the warp
  int warp_lane   = threadIdx.x / WARP_SIZE;     // warp index within the CTA
  __shared__ T count[WARPS_PER_BLOCK];
...
// set difference: c = a - b
template <typename T = vidType>
__forceinline__ __device__ T difference_set_bs_cache(T* a, T* source, T size_a, T* b, T size_b, T upper_bound, T* c) {
  //if (size_a == 0) return 0;
  int thread_lane = threadIdx.x & (WARP_SIZE-1); // thread index within the warp
  int warp_lane   = threadIdx.x / WARP_SIZE;     // warp index within the CTA
  __shared__ T count[WARPS_PER_BLOCK];
...
template <typename T = vidType>
__forceinline__ __device__ T difference_set(T* a, T* source, T size_a, T* b, T size_b, T upper_bound, T* c) {
  return difference_set_bs_cache(a, source, size_a, b, size_b, upper_bound, c);
}
