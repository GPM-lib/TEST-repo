...
__device__ struct StorageMeta {
  ...
  int local_warp_id;
#ifdef LOAD_SRAM
  int sram_lut_capacity;
  size_t sram_lut_size;
  bitmapType* lut_sram;
#endif
...
// bitmap only
  ...
  __get_vmap_from_lut(GraphGPU& g, StorageMeta& meta, vidType vidx_rowid, bool connected, bool is_warp, int bitmap_id, int upper_bound=-1) {
    ...
    // load bitmap from roaring
    if (upper_bound < 0){
      lut.load_roaring(bitmap_ptr, vidx_rowid, is_warp, lut.size_);
      auto vmap = VertexMapView(VertexArrayView(lut.vlist_, lut.size_), lut.bitmap(bitmap_ptr));
      if(!connected) vmap.use_zero();
      return vmap;
    } else {
      lut.load_roaring(bitmap_ptr, vidx_rowid, is_warp, upper_bound);
      auto vmap = VertexMapView(VertexArrayView(lut.vlist_, lut.size_), lut.bitmap(bitmap_ptr, upper_bound));
      if(!connected) vmap.use_zero();
      return vmap;
    }
  }
  ...
  __get_vmap_from_lut_vid_limit(GraphGPU& g, StorageMeta& meta, vidType vidx_rowid, bool connected, bool is_warp, int bitmap_id, int upper_bound) {
    auto lut = meta.lut;
    auto bitmap_ptr = meta.bitmap(bitmap_id);
    Bitmap1DView<> to_vmap = lut.bitmap_limit(bitmap_ptr, upper_bound);
    auto vmap = VertexMapView(VertexArrayView(lut.vlist_, lut.size_), to_vmap);
    lut.load_roaring(bitmap_ptr, vidx_rowid, is_warp, to_vmap.size());
    if(!connected) vmap.use_zero();
    return vmap;
  }
#endif
...
__difference_num_warp(VertexMapView v, VertexMapView u, vidType upper_bound) {
  if(v.use_one){
    ...
    return v.bitmap_.count_NOT_AND_NOT(u.bitmap_, upper_bound);
  }
}
...
__intersect_num_warp(VertexMapView v, VertexMapView u, vidType upper_bound) {
  if(v.use_one){
    ...
    return v.bitmap_.count_NOT_AND(u.bitmap_, upper_bound);
  }
}
