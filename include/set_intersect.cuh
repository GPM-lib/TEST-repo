...
template <typename T = vidType>
__forceinline__ __device__ T intersect_set_bs_cache(T* a, T* source, T size_a, T* b, T size_b, T* c) {
...
template <typename T = vidType>
__forceinline__ __device__ T intersect_bs_cache_thread_group(T* a, T size_a, T* b, T size_b, T* c, unsigned sync_mask) {
  //if (size_a == 0 || size_b == 0) return 0;
  int thread_lane = threadIdx.x & (THREAD_GROUP_SIZE-1); // thread index within the thread group
  int thread_group_lane   = threadIdx.x / THREAD_GROUP_SIZE;     // thread_group index within the CTA

  __shared__ T count[THREAD_GROUP_PER_BLOCK];
  __shared__ T cache[BLOCK_SIZE];
  T* lookup = a;
  T* search = b;
  T lookup_size = size_a;
  ...
  for (auto i = thread_lane; i < lookup_size; i += THREAD_GROUP_SIZE) {
    vidType key = lookup[i]; // each thread picks a vertex as the key
    int found = 0;
    if (binary_search_2phase_thread_group(search, cache, key, search_size))
      found = 1;
    unsigned active = __activemask() & sync_mask;
    unsigned mask = __ballot_sync(active, found);
    auto idx = __popc((mask << (THREAD_GROUP_SIZE-thread_lane-1)) & sync_mask);
    if (found) c[count[thread_group_lane]+idx-1] = key;
    if (thread_lane == 0) count[thread_group_lane] += __popc(mask);
  }
  return count[thread_group_lane];
}

// warp-wise intersection using hybrid method (binary search + merge)
template <typename T = vidType>
__forceinline__ __device__ T intersect_thread_group(T* a, T size_a, T *b, T size_b, T* c, unsigned sync_mask) {
  //if (size_a > ADJ_SIZE_THREASHOLD && size_b > ADJ_SIZE_THREASHOLD)
  //  return intersect_merge(a, size_a, b, size_b, c);
  //else
    return intersect_bs_cache_thread_group(a, size_a, b, size_b, c, sync_mask);
}
...
template <typename T = vidType>
__forceinline__ __device__ T intersect_num_bs_cache_thread_group(T* a, T size_a, T* b, T size_b, unsigned sync_mask) {
  if (size_a == 0 || size_b == 0) return 0;
  int thread_lane = threadIdx.x & (THREAD_GROUP_SIZE-1); // thread index within the warp
  int thread_group_lane   = threadIdx.x / THREAD_GROUP_SIZE;     // warp index within the CTA
  __shared__ T cache[BLOCK_SIZE];
  T num = 0;
  T* lookup = a;
  T* search = b;
...
// thread-group-wise intersection using hybrid method (binary search + merge)
template <typename T = vidType>
__forceinline__ __device__ T intersect_num_thread_group(T* a, T size_a, T *b, T size_b, unsigned sync_mask) {
  //if (size_a > ADJ_SIZE_THREASHOLD && size_b > ADJ_SIZE_THREASHOLD)
  //  return intersect_num_merge(a, size_a, b, size_b);
  //else
    return intersect_num_bs_cache_thread_group(a, size_a, b, size_b, sync_mask);
}
