...
template <typename T = vidType>
__forceinline__ __device__ bool linear_search_simple(T *list, T *cache, T key, T size) {
...
template <typename T = vidType>
__forceinline__ __device__ bool binary_search_2phase_thread_group(T *list, T *cache, T key, T size) {
  int p = (threadIdx.x / THREAD_GROUP_SIZE) * THREAD_GROUP_SIZE;
  int mid = 0;
  // phase 1: search in the cache
  int bottom = 0;
  int top = THREAD_GROUP_SIZE;
  while (top > bottom + 1) {
    mid = (top + bottom) / 2;
    T y = cache[p + mid];
    if (key == y) return true;
  ...
  //phase 2: search in global memory
  bottom = bottom * size / THREAD_GROUP_SIZE;
  top = top * size / THREAD_GROUP_SIZE - 1;
  while (top >= bottom) {
    mid = (top + bottom) / 2;
    T y = list[mid];
    if (key == y) return true;
