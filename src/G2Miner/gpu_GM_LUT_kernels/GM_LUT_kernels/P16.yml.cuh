...
GM_LUT_warp(vidType begin, vidType end, 
                  ...
                  AccType *counter,
                  LUTManager<> LUTs){
  __shared__ vidType list_size[WARPS_PER_BLOCK * 7];
  ...
  for(vidType v0_idx = warp_id; v0_idx < candidate_v0.size(); v0_idx += num_warps){
    ...
    for (vidType v1_idx = 0; v1_idx < candidate_v1.size(); v1_idx += 1) {
      ...
      __build_index_from_vmap(g, meta, __get_vmap_from_lut(g, meta, /*idx_id=*/v1_idx, /*connected=*/false, /*upper_bound=*/-1), /*slot_id=*/5);
      __build_index_from_vmap(g, meta, __get_vmap_from_lut(g, meta, /*idx_id=*/v1_idx, /*connected=*/true, /*upper_bound=*/-1), /*slot_id=*/6);
      ...
      for(vidType v2_idx = 0; v2_idx < candidate_v2.size(); v2_idx ++){
        ...
        __intersect(meta, __get_vmap_from_heap(g, meta, /*bitmap_id=*/-1, /*slot_id=*/6), __get_vlist_from_graph(g, meta, /*vid=*/v2), /*upper_bound=*/-1, /*output_slot=*/4);
        ...
        for(vidType v3_idx_idx = thread_lane; v3_idx_idx < candidate_v3_idx.size(); v3_idx_idx += WARP_SIZE){
          ...
          count += __difference_num(__get_vmap_from_heap(g, meta, /*bitmap_id=*/0, /*slot_id=*/-1), __get_vmap_from_lut(g, meta, /*idx_id=*/v3_idx, /*connected=*/false, /*upper_bound=*/-1), /*upper_bound=*/v1_idx);
        }
      }
    }
  }
